	...
	def Main(self):
		...
		for data in dataset_names:
			...
			if counter==0:
				...
				Velocity_X=np.zeros(len(dataset_names),Npts)
				Velocity_Y=np.zeros(len(dataset_names),Npts)
				Velocity_Z=np.zeros(len(dataset_names),Npts)
				Velocity_Mag=np.zeros(len(dataset_names),Npts)
